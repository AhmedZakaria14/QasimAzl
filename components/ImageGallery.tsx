  ...
  return (
    <>
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 mt-12 mb-16">
        {images.map((image, i) => (
          <button
            type="button"
            key={i}
            className="relative h-64 rounded-xl overflow-hidden cursor-pointer group shadow-md text-right focus:outline-none focus-visible:ring-4 focus-visible:ring-[#4a2a8a]/25"
            onClick={() => setIndex(i)}
      ...
      <Lightbox
        ...
        styles={{ container: { backgroundColor: "rgba(15, 8, 28, .96)" } }}
      />
    </>
  );
}
